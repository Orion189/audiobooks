...
const Index = memo(() => {
    ...
    const createAppDir = useCallback(async () => {
        try {
            const { exists } = await getInfoAsync(APP_DIR);

            if (!exists) {
                await makeDirectoryAsync(APP_DIR);
            }
        } catch (e) {
            NewRelic.recordError(new Error('[index] - createAppDir', e as Error));
        }
    }, [getInfoAsync, makeDirectoryAsync]);
